...
function Lightbox({ images, index, onClose, setIndex }) {
  ...
  return (
    <div style={overlay} onClick={onClose} role="dialog" aria-modal="true">
      <img src={src} alt="" style={imgStyle} onClick={(e) => e.stopPropagation()} />
      <button
        style={{
          position: 'fixed',
          top: 16,
...
function Article({ post, first = false, isShare = false }) {
  ...
  const gallery =
    Array.isArray(post.images_share) && post.images_share.length
      ? post.images_share
  ...
  return (
    <article style={{ marginBottom: 36 }}>
      ...
      <div style={{ opacity: 0.7, marginBottom: 14 }}>{fmtDate(post.date)}</div>
      ...
      {hasGallery && (
        <>
          ...
          {lbOpen && (
            <Lightbox images={gallery} index={lbIndex} setIndex={setLbIndex} onClose={() => setLbOpen(false)} />
          )}
        </>
      )}
    </article>
...
export default function NewsPost() {
  ...
  const isShare =
    location.pathname.startsWith('/share/news/') || location.pathname.startsWith('/n/')
